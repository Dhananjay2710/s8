...
class AdminDashboardController extends Controller
{
    ...
    public function update_dashboard_variant_buyer_seller(Request $request)
    {
        ...
        if (!empty($request->dashboard_variant_buyer)){
            $this->validate($request, [
                'dashboard_variant_buyer' => 'required|string'
            ]);
            update_static_option('dashboard_variant_buyer', $request->dashboard_variant_buyer);
            update_static_option('dashboard_variant_seller', $request->dashboard_variant_buyer);
        }

        if (!empty($request->start_week_from)){
            $this->validate($request, [
        ...
        return redirect()->back()->with(['msg' => __('Dashboard Variant Settings Updated..'), 'type' => 'success']);
    }
