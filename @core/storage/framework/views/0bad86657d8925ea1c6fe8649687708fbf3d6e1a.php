<footer class="footer-area gradient-footer color-two">
    <div class="footer-top padding-top-100 padding-bottom-70">
        <div class="container container-two">
            <div class="row">
                <?php echo render_frontend_sidebar('footer_two'); ?>

            </div>
        </div>
    </div>
    <div class="copyright-area style-02 copyright-border">
        <div class="container container-two">
            <div class="row align-items-center">
                <?php echo render_frontend_sidebar('copyright'); ?>

            </div>
        </div>
    </div>
</footer><?php /**PATH /var/www/pinpointplus/s8/@core/resources/views/frontend/partials/pages-portion/footers/footer-02.blade.php ENDPATH**/ ?>